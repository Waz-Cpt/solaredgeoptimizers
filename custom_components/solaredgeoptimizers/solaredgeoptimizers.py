...
# AJT: Added logger setup to replace print statements with proper logging
...
class solaredgeoptimizers:
    ...
    def requestSystemData(self, itemId):
        # AJT: Fixed endpoint URL - changed from monitoringpublic.solaredge.com/publicSystemData to monitoring.solaredge.com/systemData,
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                # AJT: Ensure we have a dictionary before accessing keys
                ...
                # AJT: Changed from direct key access to .get() for safer dictionary access
                ...
                # AJT: Added specific KeyError handling with better logging
            ...
            # AJT: Replaced print() statements with logging
    ...
    def GetThecsrfToken(self, cookies):
        ...
        # AJT: Added explicit return None if CSRF token not found
        return None
    ...
    def MakeStringFromCookie(self, cookies):
        ...
        maincookiestring = (
            maincookiestring
            # AJT: Fixed typo "concent" to "consent" in cookie string
